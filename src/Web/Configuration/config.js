(() => {
  ...
  // <<< HIER DEINE PLUGIN-GUID EINTRAGEN >>>
  ...
  function normalize(cfgIn) {
    const c = Object.assign({}, DEFAULTS, cfgIn || {});

    // Backcompat: Wenn DailyScanHour gesetzt ist, aber keine Time-String:
    ...
    // Fallbacks sicherstellen
    if (!Array.isArray(c.IncludeFolders)) c.IncludeFolders = [];
    if (!Array.isArray(c.ExcludeFolders)) c.ExcludeFolders = [];
    if (typeof c.MinItemsPerFolder !== 'number' || Number.isNaN(c.MinItemsPerFolder)) c.MinItemsPerFolder = DEFAULTS.MinItemsPerFolder;
  ...
  function applyToForm(c) {
    ...
    $(S.useBaseNameName).checked   = !!c.UseFolderBasenameAsCollectionName;
    $(S.useBaseNameFolder).checked = !!c.UseFolderBasenameAsCollectionFolder;
    ...
    $(S.dailyTime).value = (/^\d{2}:\d{2}$/.test(c.DailyScanTime||''))
      ? c.DailyScanTime
      : DEFAULTS.DailyScanTime;
  }
  ...
  function readFromForm(base) {
    ...
    const minVal = Number($(S.minItemsPerFolder).value);
    out.MinItemsPerFolder = Number.isFinite(minVal) ? Math.max(0, Math.trunc(minVal)) : DEFAULTS.MinItemsPerFolder;
  ...
  async function loadConfig() {
    ...
    try {
      ...
      _cfg = normalize(serverCfg);
      applyToForm(_cfg);
      log('Konfiguration geladen', _cfg);
    } catch (e) {
  ...
  async function saveConfig(ev) {
    ...
    try {
      ...
      log('Konfiguration gespeichert', _cfg);
    } catch (e) {
  ...
  async function scanNow() {
    ...
    try {
      ...
      // Erst moderner Endpunkt, dann Fallback:
  ...
  function wire() {
    ...
    // Beide Events unterstützen (verschiedene Jellyfin-Versionen):
