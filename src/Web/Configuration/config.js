define([], function () {
  ...
  async function getOurTaskId() {
    // alle Scheduled Tasks abrufen
    const tasks = await ApiClient.getJSON(ApiClient.getUrl('ScheduledTasks'));
    // per Key (unser Task setzt Key = "FolderCollectionsTask") oder am Namen finden
  ...
  async function updateTrigger(hour, minute) {
    ...
    const triggers = [{
      Type: 'Daily',
      TimeOfDayTicks: (hour * 3600 + minute * 60) * 10000000 // Sekunden → Ticks
    }];
  ...
  return function (view) {
    ...
    async function load() {
      try {
        ...
        Dashboard.alert({ title: 'Folder Collections', message: 'Konfiguration konnte nicht geladen werden.' });
      }
    }
    ...
    async function save(ev) {
      ...
      try {
        ...
        // Daily trigger auf neue Uhrzeit setzen
        await updateTrigger(h, m);

        Dashboard.alert({ title: 'Folder Collections', message: 'Gespeichert' });
      } catch (e) {
        console.error('Save config failed', e);
        Dashboard.alert({ title: 'Folder Collections', message: 'Speichern fehlgeschlagen.' });
      }
    }
    ...
    async function runNow() {
      try {
        await startOurTask();
        Dashboard.alert({ title: 'Folder Collections', message: 'Scan gestartet.' });
      } catch (e) {
        console.error('Run now failed', e);
        Dashboard.alert({ title: 'Folder Collections', message: 'Scan konnte nicht gestartet werden.' });
      }
    }
